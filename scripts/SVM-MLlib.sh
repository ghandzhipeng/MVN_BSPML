...
func(){
    ...
    /mnt/local/zhipeng/spark-2.1.1-bin-hadoop2.7/bin/spark-submit --master yarn\
            ...
            --conf spark.ml.sgd.optim=sgd \
            --conf spark.ml.sgd.momentum=0.9 \
            --conf spark.ml.sgd.rmsprop.epsilon=1e-7 \
            --conf spark.ml.sgd.rmsprop.forget=0.9 \
            --conf spark.ml.sgd.adam.epsilon=1e-7 \
            --conf spark.ml.sgd.adam.beta1=0.9 \
            --conf spark.ml.sgd.adam.beta2=0.99 \
            --conf "spark.executor.extraJavaOptions=-XX:+UseG1GC"\
            --executor-memory 20g\
            --class svmWithSGDMLlib /mnt/local/zhipeng/ghandMLlib.jar\
            ${inf} ${num_iter} ${mbf} ${steps} ${num_worker} ${cores_per_executor} ${partitions_per_core} ${reg_para} ${num_features}

    bash /mnt/local/zhipeng/detail-analysis/gather_mylog.sh self-logs/MLlib-${data_name}-${num_worker}-${reg_para}
#sleep 10
}
# usage:
...
num_iteration=20
mini_batch_fraction=0.001
step_size=0.01 #reg0 no scal
num_features=999990
...
step_size=0.01 #reg0, no scal 
step_size=0.01 #reg0.1, no scal 
step_size=0.0001 #reg0, scal
...
step_size=1 # reg0.1, no scal
step_size=1 # reg0, no scal
step_size=0.1 # reg0, scal
