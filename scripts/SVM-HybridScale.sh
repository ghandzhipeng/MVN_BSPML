...
func(){
    ...
    /mnt/local/zhipeng/spark-2.1.1-bin-hadoop2.7/bin/spark-submit --master yarn\
            ...
            --executor-memory 20g\
            --class svmHybridScale /mnt/local/zhipeng/ghandMLlib.jar\
            ${inf} ${num_iter} ${budget} ${steps} ${num_worker} ${cores_per_executor} ${partitions_per_core} ${reg_para} ${num_features}

    bash /mnt/local/zhipeng/detail-analysis/gather_mylog.sh self-logs/Hybrid-Scale-${data_name}-${num_worker}-${reg_para}
#sleep 10
}
# usage:
